...
from src.core.websocket.redis_socketio_manager import ConnectionManager as RedisConnectionManager
from src.core.websocket.nats_socketio_manager import ConnectionManager as NATSConnectionManager
from src.app.handlers.exception_handlers import add_exceptions_handlers
...
from src.core.opentelemetry.tracing import init_tracing
from src.core.routes_loader import auto_register_routes
from src.database.sqlalchemy.utils import sessionmanager
from src.middleware.etag_middleware import ETagMiddleware
...
from src.middleware.maintenance_mode_middleware import MaintenanceModeMiddleware
from src.middleware.monitor_performance import PerformanceMonitorMiddleware
from src.middleware.rate_limit_middleware import default_limiter
...
def setup_admin_html_routes(admin_app: FastAPI, logger: AppLogger) -> None:
    ...
    auto_register_routes(
        admin_app,
        plugin_path="src/app/routers/admin",
        prefix="/admin",
        logger=logger,
    )
...
def setup_admin_api_routes(admin_app: FastAPI, logger: AppLogger) -> None:
    ...
    auto_register_routes(
        admin_app,
        plugin_path="src/app/api/admin/v1",
        prefix="/api/v1",
        logger=logger,
    )
    auto_register_routes(
        admin_app,
        plugin_path="src/app/api/admin/v2",
        prefix="/api/v2",
        logger=logger,
    )
...
def setup_public_api_routes(public_app: FastAPI, logger: AppLogger) -> None:
    ...
    auto_register_routes(
        public_app,
        plugin_path="src/app/api/public/v1",
        prefix="/api/v1",
        logger=logger,
    )
    auto_register_routes(
        public_app,
        plugin_path="src/app/api/public/v2",
        prefix="/api/v2",
        logger=logger,
    )
    ...
    auto_register_routes(
        public_app,
        plugin_path="src/app/plugins/v1",
        prefix="/api/v1",
        logger=logger,
    )
    auto_register_routes(
        public_app,
        plugin_path="src/app/plugins/v2",
        prefix="/api/v2",
        logger=logger,
    )
...
def setup_public_html_routes(public_app: FastAPI, logger: AppLogger) -> None:
    ...
    auto_register_routes(
        public_app,
        plugin_path="src/app/routers/public",
        prefix="",
        logger=logger,
    )
